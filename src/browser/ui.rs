...
impl ChromeUi {
    ...
    fn calculate_content_area(
        ...
        let content_height = if height > ui_height {
            height - ui_height
        } else {
            0
        };
    ...
    fn clear(&mut self) {
        self.pixmap
            .fill(tiny_skia::Color::from_rgba8(255, 255, 255, 255));
    }
    ...
    fn draw_tab_bar(&mut self, tab_manager: &TabManager) {
        ...
        if let Some(rect) =
            Rect::from_xywh(0.0, 0.0, self.pixmap.width() as f32, tab_bar_height as f32)
        {
            self.pixmap
        ...
        let tab_width = (tab_area_width / tabs.len().max(1) as u32)
            .min(tab_max_width)
            .max(100);
    ...
    fn draw_tab(&mut self, tab: &crate::browser::tabs::Tab, x: i32, width: u32, is_active: bool) {
        ...
        if let Some(rect) =
            Rect::from_xywh((x + 2) as f32, 4.0, (width - 4) as f32, (height - 4) as f32)
        {
            self.pixmap
                .fill_rect(rect, &bg_paint, Transform::identity(), None);
        }
        ...
        if let Some(rect) = Rect::from_xywh(text_x as f32, text_y as f32, text_width as f32, 16.0) {
            self.pixmap
                .fill_rect(rect, &text_paint, Transform::identity(), None);
        }
    }
    ...
    fn draw_address_bar(&mut self, x: i32, y: i32, width: u32, height: u32, state: &ChromeUiState) {
        ...
        if let Some(rect) = Rect::from_xywh(x as f32, y as f32, border_width, height as f32) {
            self.pixmap
                .fill_rect(rect, &border_paint, Transform::identity(), None);
        }
        if let Some(rect) = Rect::from_xywh(
            (x + width as i32) as f32 - border_width,
            y as f32,
            border_width,
            height as f32,
        ) {
        ...
        if let Some(rect) = Rect::from_xywh(
            x as f32,
            (y + height as i32) as f32 - border_width,
            width as f32,
            border_width,
        ) {
            self.pixmap
        ...
        if let Some(rect) = Rect::from_xywh(lock_x as f32, lock_y as f32, 16.0, 16.0) {
            self.pixmap
                .fill_rect(rect, &lock_paint, Transform::identity(), None);
        }
    ...
    pub fn composite_page(&mut self, page_pixmap: &Pixmap, _content_x: u32, _content_y: u32) {
        ...
        let copy_height = page_pixmap
            .height()
            .min(self.pixmap.height().saturating_sub(ui_height));
