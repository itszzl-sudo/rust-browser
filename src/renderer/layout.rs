...
/// 布局引擎
///
/// ⚠️ 已废弃：此实现已被 `TaffyLayoutEngine` 替代。
/// 请使用 `crate::renderer::taffy_layout::TaffyLayoutEngine` 代替。
#[deprecated(note = "已被 TaffyLayoutEngine 替代，请使用 taffy_layout 模块")]
...
impl LayoutEngine {
    ...
    pub fn compute(
        &mut self,
        tree: &DomTree,
        _stylesheet: &crate::css::stylesheet::Stylesheet,
        viewport: (u32, u32),
    ) -> Result<(), String> {
        debug!("计算布局: {:?}", viewport);
